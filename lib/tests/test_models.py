...
class TestPolyModel1D:
    ...
    def test_initialize_weights(self, model_config):
        ...
        ), f"Model should have weights of shape {model_config.out_features, model_config.in_features}, is {model.weight.shape}"
        # Test initialization
        ...
        assert torch.allclose(
            winit, wtest
        ), f"Model should have weights {wtest}, is {winit}"
    ...
    def test_forward_poly_model_1d(self, model_config):
        ...
        assert (
            output.item() == w1 * w2 * input.item()
        ), f"Output should be {w1 * w2 * input.item()}, got {output.item()}"
...
class TestPolyModel2D:
    ...
    def test_initialize_weights_2d(self, model_config2d):
        ...
        assert weights.shape == (
            model_config2d.out_features,
            model_config2d.in_features,
        ), f"Model should have weights of shape {model_config2d.out_features, model_config2d.in_features}, is {weights.shape}"
